...
namespace MxMEditor
{
    ...
    *********************************************************************************************/
    public class MxMPreProcessor
    {
        ...
        *         
        *********************************************************************************************/
        private void Initialize(MxMAnimData _animData)
        {
            Assert.IsNotNull(_animData, "Cannot Pre-Process MxM data with null MxMAnimData (_animData)");
            m_processedData = _animData;
        ...
        *********************************************************************************************/
        private void PrepareDataContainers()
        {
            ...
            foreach (MxMBlendSpace blendSpace in m_animBlendSpaceList)
        ...
        *********************************************************************************************/
        private void RunSimulation()
        {
            ...
            for(int i=0; i < m_animList.Count; ++i)
            {
                ...
                //Add node's clip to clip list if necessary 
                ...
                //Genearte a list of clips to play for trajectories of this node
                ...
                for (int k=0; k < contClipList.Count; ++k)
                {
                    ...
                    else
                    {
                        ...
                        thisClipStartTime = globalPoseInterval - lastClipTimeRemainder;
                    }
                    ...
                    for (float time = thisClipStartTime; time <= thisClipEndTime; time += globalPoseInterval)
                    {
                        ...
                        m_playableGraph.Evaluate(globalPoseInterval);
                ...
                for (int k = 0; k < globalPoses.Count; k += 2)
                {
                    ...
                    float timeDelta = globalPosePlus.Time - globalPoseMinus.Time;
                    ...
                    if (timeDelta > Mathf.Epsilon)
                    {
                        ...
                        globalPosePlus.Position - globalPoseMinus.Position) / timeDelta;
                    }
                    ...
                    switch(curMxMAnim.AnimType)
                    {
                        ...
                        case EMxMAnimtype.Composite: //Choose betwen clip and composite
                            {
                    ...
                    for (int j=0; j < globalPose.JointPositions.Count; ++j)
                    {
                        ...
                        if(timeDelta > Mathf.Epsilon)
                        {
                            jointVelocity = m_helperTransform.InverseTransformVector(
                                jointPosPlus - jointPosMinus) / timeDelta;
                ...
                while(true)
                {
                    if (afterClips.Count > index || a_mxmAnim.IsLooping)
                    {
                        ...
                        if (nextClip != null)
                        {
                            ...
                            {
                                int loops = Mathf.CeilToInt(timeReq / nextClip.length);
        ...
        *********************************************************************************************/
        public ClipData ProcessClipData(IMxMAnim a_mxmAnim)
        {
            ...
            return new ClipData(startPoseId, endPoseId, clipId, clip.isLooping);
        }
        ...
        *********************************************************************************************/
        public CompositeData ProcessCompositeData(IMxMAnim a_mxmAnim)
        {
            ...
            return new CompositeData(startPoseId, endPoseId, clipIds[0], clipIds[1], primaryClip.length, afterClips[0].length);
        }
        ...
        *********************************************************************************************/
        public BlendSpaceData ProcessBlendSpaceData(IMxMAnim a_mxmAnim)
        {
            ...
            return new BlendSpaceData(startPoseId, endPoseId, blendSpace.NormalizeTime, blendSpace.Magnitude,
                 blendSpace.Smoothing, clipIds, blendSpace.Positions.ToArray());
        }
        ...
        *********************************************************************************************/
        private BlendClipData ProcessBlendClipData(IMxMAnim a_mxmAnim)
        {
            ...
            return new BlendClipData(startPoseId, endPoseId, blendClip.NormalizeTime, clipIds, blendClip.Weightings);
        }
        ...
        *********************************************************************************************/
        private void ProcessIdleSets()
        {
            ...
            for (int i = 0; i < m_animIdleSetList.Count; ++i)
            {
                ...
                for (float clipTime = 0; clipTime < idleSet.PrimaryClip.length + Mathf.Epsilon; clipTime += m_data.PoseInterval)
                {
                    ...
                    m_playableGraph.Evaluate(m_data.PoseInterval);
                }
                ...
                for (int k = 0; k < idleSet.TransitionClips.Count; ++k)
                {
                    ...
                    for (float clipTime = 0; clipTime < transitionClip.length + Mathf.Epsilon; clipTime += m_data.PoseInterval)
                    {
                ...
                for (int k = 0; k < idleSet.SecondaryClips.Count; ++k)
                {
                    ...
                    for (float clipTime = 0; clipTime < secondaryClip.length + Mathf.Epsilon; clipTime += m_data.PoseInterval)
                    {
