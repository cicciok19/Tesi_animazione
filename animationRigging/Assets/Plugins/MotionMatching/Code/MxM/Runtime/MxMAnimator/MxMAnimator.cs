...
namespace MxM
{
    ...
    *********************************************************************************************/
    ...
    public partial class MxMAnimator : MonoBehaviour
    {
        ...
        [SerializeField] private bool m_nextPoseToleranceTest = true;        //Toggle for turning on next pose tolerance testing (good enough testing)
        [SerializeField] private float m_nextPoseToleranceDist = 0.5f;       //The tolerance threshold for trajectory positions in m/s
        [SerializeField] private float m_nextPoseToleranceAngle = 5f;        //The tolerance threshold for trajectory facing angles in degrees/s
        ...
        [SerializeField] private bool m_autoCreateAnimatorController = false;       //If true, a mechanim controller will automatcially be created and added to layer 1 (second layer) of the system. Provided there is a controller sloted in the animator.
        ...
        [SerializeField] private UnityEvent m_onSetupComplete = null;                   //Unity event called when setup of the motion matching playable graph and settings is complete
        [SerializeField] private UnityEvent m_onIdleTriggered = null;                   //Unity event called when the idle state is triggered
        [SerializeField] private UnityEvent m_onEventComplete = null;                   //Unity event called when an Event (MxM Action Event) is completed
        [SerializeField] private UnityEvent_EEventState m_onEventStateChanged = null;   //Unity event called when an Event (MxM Action Event) state is changed. The event state will be passed
        [SerializeField] private UnityEvent_Int m_onEventContactReached = null;         //Unity event called whenever a contact has been reached in an event (MxM Action Event). The id of the contact will be passed
        [SerializeField] private UnityEvent_FootStepData m_onLeftFootStepStart = null;  //Unity event called when a left footstep is triggered. Footstep data will be passed
        [SerializeField] private UnityEvent_FootStepData m_onRightFootStepStart = null; //Unity event called when a right footstep is triggered. Footstep data will be passed.  
        ...
        private bool m_enforceClipChange;               //If the clip must be forceably changed this frame (used when a cut clip nears it's end)
        ...
        public MxMAnimData[] AnimData { get { return m_animData; } }
    ...
    *********************************************************************************************/
    protected virtual void OnEnable()
        ...
        *  
        *  Note: The logic in this function will onlyi run if the Animator Update mode is not set to 
        *  Animate Phsyics.
        *         
        *********************************************************************************************/
        ...
        *********************************************************************************************/
        protected virtual void OnAnimatorMove()
        {
            ...
            if (m_rootMotionMode != EMxMRootMotion.Off)
            {
                ...
                {
                    UpdateEventWarping(out warp, out warpRot);
                }
                else
                {
        ...
        *********************************************************************************************/
        protected void Initialize()
        {
            ...
            p_trajectoryGenerator = GetComponent<IMxMTrajectory>();
            ...
            //Inform the trajectory generator what the requirements are for the trajectory output
            p_trajectoryGenerator.SetGoalRequirements(CurrentAnimData.PosePredictionTimes);
        ...
        *********************************************************************************************/
        private void SetupAnimator()
        {
            ...
            m_animationLayerMixer = AnimationLayerMixerPlayable.Create(MxMPlayableGraph, 3);
            ...
            if (p_animator.runtimeAnimatorController != null)
            {
                ...
                m_animationLayerMixer.ConnectInput(1, m_animControllerPlayable, 0);
            ...
            else
            {
                ...
                startState.TargetPlayable = AnimationClipPlayable.Create(MxMPlayableGraph, clip);
        ...
        *********************************************************************************************/
        private void UpdateMatching_Phase1()
        {
            ...
            if ((m_curInterpolatedPose.GenericTags & EGenericTags.DisableMatching) != EGenericTags.DisableMatching)
            {
                ...
                if ((m_timeSinceMotionUpdate > m_updateInterval || m_enforceClipChange))
                {
        ...
        //============================================================================================
        /**
        *  @brief This function applies the results of the motion maching jobs. This is part of the
        *  second phase update. 
        *         
        *********************************************************************************************/
        ...
        *********************************************************************************************/
        ...
        {
            if(!m_enforceClipChange && m_nextPoseToleranceTest)
            {
        ...
        *********************************************************************************************/
        private void FinalizePoseSearch()
        {
            ...
            m_minimaJobHandle = GenerateMinimaJob(m_curInterpolatedPose.PoseId,
                m_enforceClipChange, CurrentNativeAnimData.UsedPoseIds.Length);
            ...
            if (!winnerAtSameLocation || m_enforceClipChange)
            {
        ...
        private void UpdateBlending()
        {
            ...
            for (int i = 0; i < m_animationStates.Length; ++i)
            {
                ...
                else
                {
                    ...
                    if (animBlendVal < p_currentDeltaTime)
                    {
#if UNITY_2019_1_OR_NEWER
                        if (p_riggingIntegration != null)
        ...
        *********************************************************************************************/
        ...
        {
            if (Mathf.Abs(DesiredPlaybackSpeed - m_playbackSpeed) > 0.001f)
            {
                m_playbackSpeed = Mathf.MoveTowards(m_playbackSpeed, DesiredPlaybackSpeed,
        ...
        *********************************************************************************************/
        public float ComputePoseCost(ref PoseData a_candidate)
        {
            ...
            //Needs to also compensate for velocity of the aspect (higher velocity allows greater
            //discrepancy in position and velocity
...
//#endif
        ...
        //============================================================================================
        /**
        *  @brief This function tessts if the next pose is close enough in trajectory to the current
        ...
        *********************************************************************************************/
        private bool CloseEnoughTest(ref PoseData a_nextPose)
        {
            ...
            if (a_nextPose.Tags != m_requiredTags)
                return false;
        ...
        *         
        *********************************************************************************************/
        private void TransitionToPose(ref PoseData a_pose, float a_speedMod = 1f)
        {
            switch(m_transitionMethod)
            {
                case ETransitionMethod.None: { JumpToPose(ref a_pose, a_speedMod); } break;
                case ETransitionMethod.Blend: { BlendToPose(ref a_pose, a_speedMod); } break;
        ...
        *         
        *********************************************************************************************/
        private void JumpToPose(ref PoseData a_pose, float a_speedMod = 1f)
        {
            int clipId = a_pose.PrimaryClipId;

            if (clipId < CurrentAnimData.Clips.Length && clipId > -1)
            {
                for (int i = 0; i < m_animationStates.Length; ++i)
                {
                    ...
                    m_animationMixer.SetInputWeight(i, 0f);
                    m_animationMixer.DisconnectInput(i);

                    if (curPlayableState.TargetPlayable.IsValid())
                        curPlayableState.TargetPlayable.Destroy();
                }
        ...
        *         
        *********************************************************************************************/
        private void BlendToPose(ref PoseData a_pose, float a_speedMod = 1f)
        {
            if (a_pose.AnimId < 0)
                return;

            ref MxMPlayableState primaryState = ref m_animationStates[m_primaryBlendChannel];
            primaryState.DecayAge = 0f;
            ...
            SetupPoseInSlot(ref a_pose, channelToUse, a_speedMod);
        ...
        *********************************************************************************************/
        public void ResetPlayableGraph()
        {
            if (!m_animationLayerMixer.IsNull())
            {
                ...
                m_animationLayerMixer.SetInputCount(3);
            }
        }
        ...
        *********************************************************************************************/
        private void FetchNativeAnimData()
        {
            ...
            if (CurrentAnimData.NativeAnimData.TryGetValue(m_requiredTags, out nativeAnimData))
            {
                CurrentNativeAnimData = nativeAnimData;
            }
            else
            {
                Debug.LogError("MxMAnimator - cannot find native animator to suite tags: " + m_requiredTags.ToString());
            }
        }
        ...
        *********************************************************************************************/
        ...
        {
            if (a_calibDataId >= 0 && a_calibDataId < CurrentAnimData.CalibrationSets.Length)
            {
        ...
        *********************************************************************************************/
        public void SetAngularErrorWarping(EAngularErrorWarp a_type, EAngularErrorWarpMethod a_method = EAngularErrorWarpMethod.CurrentHeading,
            float a_compensationRate = 45f, float a_distThreshold = 0.5f, float a_angleThreshold = 45f)
        {
            //m_applyDiscrepComp = a_on;
