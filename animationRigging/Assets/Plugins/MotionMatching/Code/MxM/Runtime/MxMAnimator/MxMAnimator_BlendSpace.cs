    ...
    *********************************************************************************************/
    public partial class MxMAnimator : MonoBehaviour
    {
        ...
        *********************************************************************************************/
        ...
        {
            if (m_dominantPose.AnimId >= CurrentAnimData.BlendSpaces.Length || m_dominantPose.AnimId < 0)
                return;

            ref readonly BlendSpaceData curBlendSpace = ref CurrentAnimData.BlendSpaces[m_dominantPose.AnimId];
            ...
            CalculateBlendSpaceWeightings(curBlendSpace.Positions);

            var blendSpacePlayable = m_animationMixer.GetInput(m_dominantBlendChannel);
            ...
            if (blendSpacePlayable.IsValid())
            {
                ...
                {
                    var clipPlayable = blendSpacePlayable.GetInput(i);
        ...
        *********************************************************************************************/
        public bool BeginLoopBlend(int a_blendSpaceId, float a_x = 0f, float a_y = 0f)
        {
            if (a_blendSpaceId >= 0 && a_blendSpaceId < CurrentAnimData.BlendSpaces.Length)
            {
                ...
                //Use a layer for the blend space but keep the primary animation blended in.
        ...
        *         
        *********************************************************************************************/
        public void SetBlendSpacePosition(float a_x, float a_y)
        {
            if (m_chosenPose.AnimType == EMxMAnimtype.BlendSpace)
            {
                ...
                if (m_blendSpaceSmoothing != EBlendSpaceSmoothing.None)
                    return;
                ...
                ref readonly BlendSpaceData curBlendSpace = ref CurrentAnimData.BlendSpaces[m_chosenPose.AnimId];

                CalculateBlendSpaceWeightings(curBlendSpace.Positions);

                var blendSpacePlayable = m_animationMixer.GetInput(m_curBlendSpaceChannel);
        ...
        *         
        *********************************************************************************************/
        public void SetBlendSpacePositionX(float a_x)
        {
            if (m_chosenPose.AnimType == EMxMAnimtype.BlendSpace)
                m_desiredBlendSpacePosition.x = Mathf.Clamp(a_x, -1f, 1f);
        ...
        *         
        *********************************************************************************************/
        public void SetBlendSpacePositionY(float a_y)
        {
            if (m_chosenPose.AnimType == EMxMAnimtype.BlendSpace)
            {
                m_desiredBlendSpacePosition.y = Mathf.Clamp(a_y, -1f, 1f);
        ...
        *         
        *********************************************************************************************/
        public void ResetBlendSpacePositions()
        {
            if (m_chosenPose.AnimType == EMxMAnimtype.BlendSpace)
            {
            ...
            switch (m_blendSpaceSmoothing)
            {
                case EBlendSpaceSmoothing.None:
                    m_blendSpacePosition.y = m_desiredBlendSpacePosition.y;
                    break;
                default:
                    return;
            }
