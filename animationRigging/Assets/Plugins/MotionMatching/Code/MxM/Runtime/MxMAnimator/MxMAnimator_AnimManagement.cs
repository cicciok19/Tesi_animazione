    ...
    public partial class MxMAnimator : MonoBehaviour
    {
        ...
        *********************************************************************************************/
        private void SetupClipInSlot(ref PoseData a_pose, int a_slotId, float a_speedMod = 1f)
        {
            ...
            playableState.SetAsChosenWithPose(ref a_pose);
            ...
            playableState.TargetPlayable.SetTime(a_pose.Time);
            playableState.TargetPlayable.SetTime(a_pose.Time);
        ...
        *  
        *  Instead of creating and destorying the appropriate playables, this function simply gives 
        *  the correct clip a correct weight.
        *         
        *********************************************************************************************/
        private void SetupClip(ref PoseData a_pose, float a_speedMod = 1f)
        {
            m_animationMixer.SetInputWeight(a_pose.PrimaryClipId, 1f);
            var clipPlayable = m_animationMixer.GetInput(a_pose.PrimaryClipId);
            clipPlayable.SetTime(a_pose.Time);
            clipPlayable.SetTime(a_pose.Time);
        ...
        //============================================================================================
        /**
        *  @brief Sets up a composite (complex anim) in an input slot on the motion matchin mixer.
        *  
        ...
        *         
        *********************************************************************************************/
        private void SetupCompositeInSlot(ref PoseData a_pose, int a_slotId, float a_speedMod)
        {
            ref MxMPlayableState playableState = ref m_animationStates[a_slotId];
            playableState.SetAsChosenWithPose(ref a_pose);

            CompositeData composite = CurrentAnimData.Composites[a_pose.AnimId];
            ...
            if (a_pose.Time < composite.ClipALength)
            {
                ...
                clipPlayableA.SetTime(a_pose.Time);
                clipPlayableA.SetTime(a_pose.Time);
                clipPlayableA.SetSpeed(m_playbackSpeed * a_speedMod);
            ...
            else
            {
                ...
                playableState.TargetPlayable = AnimationClipPlayable.Create(MxMPlayableGraph, clipB);

                playableState.TargetPlayable.SetTime(a_pose.Time - composite.ClipALength);
                playableState.TargetPlayable.SetTime(a_pose.Time - composite.ClipALength);
                playableState.TargetPlayable.SetSpeed(m_playbackSpeed * a_speedMod);
        ...
        *         
        *********************************************************************************************/
        private void SetupComposite(ref PoseData a_pose, float a_speedMod)
        {
        ...
        //============================================================================================
        /**
        *  @brief Sets up a blend space (complex anim) in an input slot on the motion matchin mixer.
        *  
        ...
        *         
        *********************************************************************************************/
        private void SetupBlendSpaceInSlot(ref PoseData a_pose, int a_slotId, float a_speedMod)
        {
            ref MxMPlayableState playableState = ref m_animationStates[a_slotId];
            playableState.SetAsChosenWithPose(ref a_pose);
            ...
            for(int i = 0; i < blendSpace.ClipIds.Length; ++i)
            {
                ...
                blendClipPlayable.SetTime(a_pose.Time * normalizedClipSpeed);
                blendClipPlayable.SetTime(a_pose.Time * normalizedClipSpeed);
                blendClipPlayable.SetSpeed(m_playbackSpeed * a_speedMod * normalizedClipSpeed);
        ...
        *         
        *********************************************************************************************/
        private void SetupBlendSpace(ref PoseData a_pose, float a_speedMod)
        {
            ref readonly BlendSpaceData blendSpace = ref CurrentAnimData.BlendSpaces[a_pose.AnimId];
            ...
            for (int i = 0; i < blendSpace.ClipIds.Length; ++i)
            {
                ...
                clipPlayable.SetTime(a_pose.Time * normalizedClipSpeed);
                clipPlayable.SetTime(a_pose.Time * normalizedClipSpeed);
                clipPlayable.SetSpeed(m_playbackSpeed * a_speedMod * normalizedClipSpeed);
        ...
        //============================================================================================
        /**
        *  @brief Sets up a blend clip (complex anim) in an input slot on the motion matchin mixer.
        *  
        ...
        *         
        *********************************************************************************************/
        private void SetupBlendClipInSlot(ref PoseData a_pose, int a_slotId, float a_speedMod)
        {
            ref MxMPlayableState playableState = ref m_animationStates[a_slotId];
            playableState.SetAsChosenWithPose(ref a_pose);
        ...
        *         
        *********************************************************************************************/
        private void SetupBlendClip(ref PoseData a_pose, float a_speedMod = 1f)
        {
            ref readonly BlendClipData blendClip = ref CurrentAnimData.BlendClips[a_pose.AnimId];
