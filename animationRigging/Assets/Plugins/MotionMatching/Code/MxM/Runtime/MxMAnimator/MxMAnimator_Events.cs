...
namespace MxM
{
    ...
    *********************************************************************************************/
    public partial class MxMAnimator : MonoBehaviour
    {
        ...
        private float m_timeSinceEventTriggered; //The time passed (warped time) since the event was triggered
        ...
        private void UpdateEvent()
        {
            ...
            switch (m_curEventState)
            {
                case EEventState.Windup: //Update the windup phase of the event
                    {
                        if (m_timeSinceMotionChosen >= m_curEvent.Windup - m_eventStartTimeOffset)
                        {
                ...
                case EEventState.Action: //Update the action phase of the event
                    {
                        if (m_timeSinceMotionChosen >= m_curEventContactTime)
                        {
                ...
                case EEventState.FollowThrough: //Update the follow through phase of the event
                    {
                        ...
                        if (m_timeSinceMotionChosen >= timePassed)
                        {
                            if (m_eventType == EMxMEventType.Sequence)
                            {
                                float rewindAmount = m_timeSinceMotionChosen - timePassed + m_curEvent.FollowThrough + m_curEvent.Actions[0]; //TODO: Make this work with multi-contacts
                                ...
                                m_timeSinceMotionChosen -= rewindAmount;
                ...
                case EEventState.Recovery: //Update the recovery phase of the event
                    {
                        ...
                        if (m_timeSinceMotionChosen >= timePassed)
                        {
                            if (m_eventType == EMxMEventType.Loop)
                            {
                                ...
                                m_timeSinceMotionChosen -= timePassed;
                            ...
                            else
                            {
                                ...
                                //if(m_exitEventWithMotion)
                                //    PostEventTrajectoryMode = EPostEventTrajectoryMode.Reset;
        ...
        *********************************************************************************************/
        private void SnapToContact()
        {
            ...
            if (WarpType > EEventWarpType.Linear)
            {
            ...
            if (RotWarpType > EEventWarpType.Linear)
            {
                if (m_rootMotion != null)
                {
        ...
        *         
        *********************************************************************************************/
        private void UpdateEventWarping(out Vector3 warp, out Quaternion warpRot)
        {
            warp = Vector3.zero;
            warpRot = Quaternion.identity;

            EventFrameData lookupData;
            FetchEventLookupData(out lookupData);

            //Rotational warping
            if (lookupData.WarpRotThisFrame)
            {
                float remainingWarpTime = lookupData.RemainingRotWarpTime / m_playbackSpeed;

                //Recalculate the current root rotation at the next contact (Without warping)
                if ((int)RotWarpType > (int)EEventWarpType.Snap)
                    m_currentEventRootWorld.RotationY = m_animationRoot.rotation.eulerAngles.y + lookupData.relativeContactRootRotY;
                
                //Rotational Warping
                switch (RotWarpType)
                {
                    case EEventWarpType.Linear: { warpRot = ComputeLinearEventRotationWarping(remainingWarpTime); } break;
                    case EEventWarpType.Dynamic: { warpRot = ComputeSimpleDynamicEventRotationWarping(remainingWarpTime); } break;
                }
            }

            //Recalculate the current root position at the next contact (without warping) 
            if ((int)WarpType > (int)EEventWarpType.Snap)
                m_currentEventRootWorld.Position = m_animationRoot.TransformPoint(lookupData.relativeContactRoot);

            //Positional Warping
            if (lookupData.WarpPosThisFrame)
            {
                float remainingWarpTime = lookupData.RemainingWarpTime / m_playbackSpeed;

                switch (WarpType)
                {
                    case EEventWarpType.Linear: { warp = ComputeLinearEventWarping(remainingWarpTime); } break;
                    case EEventWarpType.Dynamic: { warp = ComputeSimpleDynamicEventWarping(remainingWarpTime); } break;
                }
            }
        }
        ...
        *********************************************************************************************/
        private void FetchEventLookupData(out EventFrameData lookupData)
        {
            ...
            float lookupTableFloatIndex = (m_timeSinceMotionChosen + m_eventStartTimeOffset) / (1f / 60f);
            ...
            if (a_eventDefinition.Priority < 0 || a_eventDefinition.Priority > m_curEventPriority || m_fsm.CurrentStateId != (uint)EMxMStates.Event)
            {
                ...
                for (int index = 0; index < CurrentAnimData.Events.Length; ++index)
                {
                    ...
                    for (int i = 0; i < evt.WindupPoseContactOffsets.Length; ++i)
                    {
                        ...
                        {
                            cost += ComputePoseCost(ref CurrentAnimData.Poses[evt.StartPoseId + i]);
                        }

                        if (a_eventDefinition.MatchTrajectory)
                        {
                            cost += ComputeTrajectoryCost(ref CurrentAnimData.Poses[evt.StartPoseId + i]);
                        }
                ...
                {
                    PlaybackSpeed = m_curEventContactTime / desiredDelay;
                }
                ...
                if (m_transitionMethod == ETransitionMethod.Blend)
                    BlendToPose(ref m_chosenPose, m_playbackSpeed);
                else
                    JumpToPose(ref m_chosenPose, m_playbackSpeed);
